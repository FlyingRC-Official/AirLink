...
fixed to the `v0.2.1-dev` ESP32-C5 N8R8 release. The page verifies the GitHub
...
2. Double-click `start_flasher.command`. If macOS blocks the launcher, right-click
   `AirLink-Flasher-v0.2.1-dev.html`, choose **Open With**, then select Google
...
`www/AirLink-Flasher-v0.2.1-dev.html`. Run `npm run dev` for local development.
The source firmware copies under `public/firmware/v0.2.1-dev` exist only so the
...
- Fixed `v0.2.1-dev` GitHub Release metadata and prerelease checks.
