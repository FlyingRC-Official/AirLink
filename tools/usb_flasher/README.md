...
fixed to the `v0.3.1-dev` ESP32-C5 N8R8 release. The page verifies the GitHub
...
`www/AirLink-Flasher-v0.3.1-dev.html`. Run `npm run dev` for local development.
The source firmware copies under `public/firmware/v0.3.1-dev` exist only so the
...
- Fixed `v0.3.1-dev` GitHub Release metadata and prerelease checks.
